...
WorkingDirName=test_dask_workflow
...
PYTHONPATH=/gpfs/alpine/bif135/proj-shared/rbd_work/dask_testing/tmalign_andes_workflow:$PYTHONPATH

# set active directories
RUN_DIR=/gpfs/alpine/bif135/proj-shared/rbd_work/dask_testing/tmalign_andes_workflow/$WorkingDirName
SCHEDULER_FILE=${RUN_DIR}/scheduler_file.json
SRC_DIR=/gpfs/alpine/bif135/proj-shared/rbd_work/dask_testing/tmalign_andes_workflow
...
if [ $nWorkersPrimary -ne 0 ]; then
	echo "Spinning $nWorkersPrimary workers up on the head node, each getting $nWorkerCores"
...
if [ $nWorkers -ne 0 ]; then
	echo "Spinning $nWorkers workers up on the head node, each getting $nWorkerCores"
...
srun -n 1 -N 1 -c $nClientCores --cpu-bind=threads -w $PrimaryNode \
	python3 ${SRC_DIR}/dask_tskmgr.py --scheduler-file $SCHEDULER_FILE \
					  --input-pdb-list ${SRC_DIR}/small_testing_set.lst \
					  --library-pdb-directory ${SRC_DIR}/testing_library/ \
					  --working-dir ${RUN_DIR} \
					  --timings-file ${RUN_DIR}/timings.csv \
					  --script-path $SRC_DIR/run_TMalign1toM_ed2.sh \
